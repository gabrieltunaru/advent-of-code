...
object Day_12_2:
  ...
  def isValid(curr: Point, next: Point, matrix: List[List[Char]]): Boolean =
    ...
    val res = nextElevation >= currElevation || currElevation == nextElevation + 1
    res
  ...
  def main(args: Array[String]): Unit =
    ...
    println(start)
    val (distance, predecessor) = BellmanFord(graph, end)

    val startPoints = distance.filter((p,d) => parsed(p.x)(p.y)=='a')

    println(distance(start))

    println(startPoints.values.min)
